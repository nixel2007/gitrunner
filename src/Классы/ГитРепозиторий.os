...
//   ПутьКФайлуКоммита - Строка - путь к файлу с текстом комментария
//   АвторКоммита - Строка - Автор комментария, передается в случае необходимости
//   ДатаАвтораКоммита - Дата - Дата комментария
...
//
Процедура Отправить(Знач ИмяРепозитория, 
    Знач ЗаголовкиОтправки = "",
	Знач Принудительно = Ложь,
    Знач Отслеживать = Ложь,
    Знач РежимОтправкиВсехВеток = Ложь,
	Знач РежимТишины = Ложь,
    Знач РежимУдаления = Ложь,
    Знач РежимПарсингаОтвета = Ложь,
	Знач РежимОтправкиТэгов = Ложь,
	Знач РежимЗеркала = Ложь) Экспорт
	...
	Если Отслеживать Тогда
		 ПараметрыЗапуска.Добавить("--set-upstream");
    КонецЕсли;

    Если ЗначениеЗаполнено(ЗаголовкиОтправки) Тогда
	     ПараметрыЗапуска.Добавить(ЗаголовкиОтправки);
    КонецЕсли;

    Если РежимОтправкиВсехВеток Тогда
        ПараметрыЗапуска.Добавить("--all");
    КонецЕсли;

	Если РежимОтправкиТэгов Тогда
        ПараметрыЗапуска.Добавить("--tags");
    КонецЕсли;
	
	Если РежимЗеркала Тогда
        ПараметрыЗапуска.Добавить("--mirror");
    КонецЕсли;
    
    Если РежимУдаления Тогда
        ПараметрыЗапуска.Добавить("--delete");
    КонецЕсли;

	Если РежимПарсингаОтвета Тогда
        ПараметрыЗапуска.Добавить("--porcelain");
    КонецЕсли;
    
    Если РежимТишины Тогда
        ПараметрыЗапуска.Добавить("--quiet");
    КонецЕсли;
...
Функция НастройкиРежимаОтправки() Экспорт
    
	ПутьКСкрипту = ОбъединитьПути(ТекущийКаталог(), "src", "Классы", "НастройкаРежимаОтправки.os");
	ПодключитьСценарий(ПутьКСкрипту, "НастройкаРежимаОтправки");
	НастройкаРежима = Новый НастройкаРежимаОтправки;
	НастройкаРежима.УстановитьГитРепозиторий(ЭтотОбъект);	
    
	Возврат НастройкаРежима;
...
//   РежимУстановкиНастроек - РежимУстановкиНастроек - Режим установки настройки.
//
Процедура УдалитьСекциюНастроек(Знач ИмяСекции, Знач РежимУстановкиНастроек = Неопределено) Экспорт
...
Процедура УстановитьКоммитера(Знач Коммитер, ТребуетсяВосстановлениеНастроек = Ложь)
    ...
    УстановитьНастройку(НастройкаНаименования, Наименование, РежимУстановкиНастроек.Локально);
    УстановитьНастройку(НастройкаПочты, Почта,  РежимУстановкиНастроек.Локально);
...
Процедура ВосстановитьКоммитера()
    ...
    Если ТекущееНаименование <> Наименование_bak Тогда 
        УстановитьНастройку(НастройкаНаименования, Наименование_bak, РежимУстановкиНастроек.Локально); 
    КонецЕсли;

    Если ТекущаяПочта <> Почта_bak Тогда 
        УстановитьНастройку(НастройкаПочты, Почта_bak, РежимУстановкиНастроек.Локально); 
    КонецЕсли;

    УдалитьСекциюНастроек("bak.user", РежимУстановкиНастроек.Локально);
