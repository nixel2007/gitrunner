...
Перем ЭтоWindows;
Перем мНастройкиРежимаОтправки;
...
Процедура Отправить(Знач ИмяРепозитория = "", Знач ИмяВетки = "", Знач ПерезаписатьИсторию = Ложь) Экспорт
    ...
    СтандартнаяНастройка = мНастройкиРежимаОтправки = Неопределено; 
   
    Если СтандартнаяНастройка Тогда
        НастройкиРежимаОтправки = Новый НастройкаРежимаОтправкиGit;
        
    Иначе
        НастройкиРежимаОтправки = мНастройкиРежимаОтправки;
    КонецЕслИ;

    Если ПерезаписатьИсторию Тогда
        НастройкиРежимаОтправки.ПерезаписатьИсторию();
    КонецЕсли;
    
    Если ЗначениеЗаполнено(ИмяРепозитория) Тогда
        НастройкиРежимаОтправки.УстановитьРепозиторийОтправки(ИмяРепозитория);
    КонецЕсли;
    
    Если ЗначениеЗаполнено(ИмяВетки) 
        И СтандартнаяНастройка Тогда
        НастройкиРежимаОтправки.УстановитьВеткуОтправки(ИмяВетки);
    КонецЕсли;
    
    ПараметрыЗапуска = НастройкиРежимаОтправки.ПолучитьПараметрыКоманды();
	    
    ВыполнитьКоманду(ПараметрыЗапуска);
    
КонецПроцедуры

Процедура УстановитьНастройкиРежимаОтправки(НастройкиРежимаОтправки) Экспорт
    
	мНастройкиРежимаОтправки = НастройкиРежимаОтправки;
...
Процедура Инициализация()
    ...
    ПутьКСкрипту = ОбъединитьПути(ТекущийКаталог(), "src", "Классы", "НастройкаРежимаОтправкиGit.os");
	ПодключитьСценарий(ПутьКСкрипту, "НастройкаРежимаОтправкиGit");
	
    мНастройкиРежимаОтправки = Неопределено;
