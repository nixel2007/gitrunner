...
Процедура ПередВыполнением_СтандартнаяОбработка()
	...
	Если Не ПроверитьМетодКласса(КлассНастройкиКоманды, ТиповаяФункция,, Истина) Тогда
		ВызватьИсключение СтрШаблон("У класса %1 не реализована обязательная функция: %2", КлассНастройкиКоманды, ТиповаяФункция);
	КонецЕсли;
...
Функция ПроверитьМетодКласса(Знач КлассРеализацииКоманды, Знач ИмяМетода, Знач ТребуемоеКоличествоПараметров = 0, Знач ЭтоФункция = Ложь)
