...
Сценарий: Отправка зафиксированных изменений через настройку отправки
    Допустим Я добавляю внешний репозиторий "origin" по сохраненному адресу внешнего репозитория
    И Я перехожу на ветку "test" с ее созданием
    И Я создаю временный файл "temp.tmp" во временном каталоге и сохраняю его в контекст
    И Я добавляю временный файл в индекс
    И Я фиксирую изменения с сообщением "Добавлен тестовый файл"
    И Я создаю новый объект НастройкаРежимаОтправки
    И Я устанавливаю настройку отправки режима тишины в значение "true"
    И Я устанавливаю настройку отправки полная отправка в значение "true"
    И Я устанавливаю настройку отправки отображение прогресса в значение "true"
    Когда Я отправляю изменения через настройку режима отправки
    И Я выполняю произвольную команду git "branch -a"
    И Я сообщаю вывод команды
    Тогда Вывод команды содержит "remotes/origin/test"
    И Я устанавливаю настройку отправки произвольный заголовок "push origin :test"
